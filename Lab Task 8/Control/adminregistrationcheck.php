...
if(empty($fname) || strlen($fname)<2 || empty($lname) || strlen($lname)<2 || empty($uname) || strlen($uname)<5 || 
   ...
   empty($password) || strlen($password)<8 || $password != $confirmpassword || $resultstring == $email)
{
    echo "No data Saved";
}
    ...
    $mydb=new DB();
    $connobj=$mydb->opencon();
    $mydb->InsertData($fname, $lname, $uname, $phone, $email, $password, $role, $filename, "admin", $connobj);
    $mydb->closecon($connobj);
